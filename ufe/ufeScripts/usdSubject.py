...
class _StagesSubject(object):
    ...
    def _afterOpenCb(self, data):
        ...
        _stageToPath.clear()
        proxyShapeNames = cmds.ls(type=usdRunTime.kMayaUsdGatewayNodeType)
        proxyShapes = [AL_USDMaya.ProxyShape.getByName(psn) for psn in
                       proxyShapeNames]
        dagPaths = [utils.nameToDagPath(psn) for psn in proxyShapeNames]
        ufePaths = [dagPathToUfe(dagPath) for dagPath in dagPaths]

        for ps, ufePath in zip(proxyShapes, ufePaths):
            _stageToPath[ps.getUsdStage()] = ufePath
            usdRunTime.pathToStage[ufePath] = ps.getUsdStage()
